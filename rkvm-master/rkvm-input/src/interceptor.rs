...
impl Interceptor {
    ...
    pub(crate) async fn open(path: &Path, registry: &Registry) -> Result<Self, OpenError> {
        ...
        let bustype = unsafe { glue::libevdev_get_id_bustype(evdev.as_ptr()) };
        if bustype == glue::BUS_VIRTUAL as i32 {
            tracing::debug!(path = %path.display(), "Skipping virtual clone (uinput)");
        ...
        if ret < 0 {
            ...
            let err = if ret == -libc::EBUSY || ret == -libc::EACCES || ret == -libc::EPERM {
                tracing::info!(
                    "Ignored {:?} because it cannot be grabbed ({})",
                    path,
                    Error::from_raw_os_error(-ret)
                );
